...
\usepackage{bm}
\usepackage{graphicx}
\graphicspath{{./}{Recurrent Neural Networks (RNNs), and the Exploding and Vanishing Gradient Problems/}{../MSSV_Ten_StockPredictionRNN/results/}}
\usepackage{tikz}
...
\begin{frame}[fragile]{Demo: Kiến trúc RNN \& dữ liệu}
    \textbf{Bài toán}
    \begin{itemize}
        \item Dự đoán giá đóng cửa cổ phiếu (Close price) dựa trên \(N\) ngày trước (\textit{lookback} \(= 20\)).
        \item Bài toán time-series regression (sequence \(\rightarrow\) one).
    \end{itemize}
    \medskip
    \textbf{Tiền xử lý dữ liệu}
    \begin{itemize}
        \item Min--Max Scaling đưa giá về \([0,1]\):
        \[
            x_{\text{norm}} = \frac{x - x_{\min}}{x_{\max} - x_{\min}}.
        \]
        \item Tạo sliding window:
        \begin{itemize}
            \item Input: \([x_{t-19}, \ldots, x_t]\).
            \item Target: \(x_{t+1}\).
        \end{itemize}
    \end{itemize}
    \medskip
    \textbf{Kiến trúc Traditional RNN}
    \begin{itemize}
        \item Input size: 1 (giá), Hidden size: \(H\), Output size: 1 (giá dự đoán).
        \item Không dùng LSTM / GRU, forward thủ công.
    \end{itemize}
    \vspace{0.3em}
    \scriptsize
\begin{verbatim}
...
x_win, y_next = X[0], y[0]  # window [x_{t-19},...,x_t], target x_{t+1}
\end{verbatim}
\end{frame}

\begin{frame}[fragile]{Demo: Forward, loss \& huấn luyện}
...
clip = clip_gradients(grads, tau)[0]; apply_gradients(model.parameters(), clip, lr)
\end{verbatim}
\end{frame}

\begin{frame}{Demo: Loss và độ lớn gradient (có clipping)}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{center}
                \includegraphics[width=\linewidth]{loss_curve_clipped.png}
            \end{center}
            \vspace{-0.5em}
            \small Loss giảm ổn định sau khi áp dụng clipping.
        \end{column}
        \begin{column}{0.48\textwidth}
            \begin{center}
                \includegraphics[width=\linewidth]{gradients_norm.png}
            \end{center}
            \vspace{-0.5em}
            \small Chuẩn gradient được giữ dưới ngưỡng \(\tau\), tránh exploding.
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Demo: Dự đoán giá cổ phiếu}
    \begin{itemize}
        \item So sánh giá thật và giá dự đoán sau khi huấn luyện với Gradient Clipping.
    \end{itemize}
    \begin{center}
        \includegraphics[width=0.85\linewidth]{prediction_chart.png}
    \end{center}
\end{frame}
