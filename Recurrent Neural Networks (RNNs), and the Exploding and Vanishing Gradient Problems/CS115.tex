...
\usepackage{bm}
\usepackage{graphicx}
\graphicspath{{./}{Recurrent Neural Networks (RNNs), and the Exploding and Vanishing Gradient Problems/}{MSSV_Ten_StockPredictionRNN/results/}{../MSSV_Ten_StockPredictionRNN/results/}}
\usepackage{listings}
\lstset{
...
\author{Le Quang Trung, Nguyen Ngoc Hung, Tran Le Anh Pha}
\date{\today}
...
\begin{frame}{Demo: RNN dự đoán giá cổ phiếu}
    \begin{itemize}
        \item Bài toán many-to-one: mỗi cửa sổ 20 điểm giá $\to$ dự báo giá tiếp theo (1 giá trị).
        \item Kiến trúc: 1 input, 16 hidden (tanh), 1 output tuyến tính.
        \item Loss huấn luyện: MSE trên output cuối chuỗi; chuẩn hóa min-max fit trên train, áp dụng cho test.
        \item Cấu hình: 300 epochs, learning rate 0.01, gradient clipping $\tau = 5$.
        \item Kết quả test: RMSE $\approx 1.45$, MAE $\approx 1.19$.
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{Code huấn luyện RNN (1/2)}
    \lstinputlisting[language=Python, firstline=19, lastline=36]{../MSSV_Ten_StockPredictionRNN/src/train.py}
\end{frame}

\begin{frame}[fragile]{Code huấn luyện RNN (2/2)}
    \lstinputlisting[language=Python, firstline=37, lastline=46]{../MSSV_Ten_StockPredictionRNN/src/train.py}
\end{frame}
...
\begin{frame}{Training loss và norm gradient (đã clipping)}
    ...
    \begin{minipage}{0.48\linewidth}
        \centering
        \IfFileExists{../MSSV_Ten_StockPredictionRNN/results/training_loss.png}{%
            \includegraphics[width=\linewidth]{../MSSV_Ten_StockPredictionRNN/results/training_loss.png}%
        }{%
    ...
    \begin{minipage}{0.48\linewidth}
        \centering
        \IfFileExists{../MSSV_Ten_StockPredictionRNN/results/gradient_norm_log.png}{%
            \includegraphics[width=\linewidth]{../MSSV_Ten_StockPredictionRNN/results/gradient_norm_log.png}%
        }{%
...
\begin{frame}{Dự báo trên tập test}
    \begin{itemize}
        ...
        \item Sai số: RMSE $\approx 1.45$, MAE $\approx 1.19$.
    \end{itemize}
    \begin{center}
        \IfFileExists{../MSSV_Ten_StockPredictionRNN/results/test_prediction_chart.png}{%
            \includegraphics[width=0.8\linewidth]{../MSSV_Ten_StockPredictionRNN/results/test_prediction_chart.png}%
        }{%
